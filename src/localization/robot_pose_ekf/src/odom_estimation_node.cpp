...
namespace estimation
{
      ...
      ekf_sent_counter_(0)
  {
    ...
    my_filter_.setOutputFrame(output_frame_);  // ekf filter  OdomEstimation my_filter_;
   ...
   //广播(advertise)要发布的话题，记录滤波开始的时间，
    ...
    if (odom_used_){
      ...
      odom_sub_ = nh.subscribe("/ndt_odometry", 10, &OdomEstimationNode::odomCallback, this);
    }
    ...
    if (vo_used_){
      ...
      vo_sub_ = nh.subscribe("/ndt_odometry", 10, &OdomEstimationNode::voCallback, this);
    }
  ...
  void OdomEstimationNode::spin(const ros::TimerEvent& e)
  {
    ...
    if ((odom_active_ || odom_initializing_) &&  (Time::now() - odom_time_).toSec() > timeout_)
    {
      odom_active_ = false; odom_initializing_ = false;
      ROS_INFO("Odom sensor not active any more");
    }
    if ((imu_active_ || imu_initializing_) && (Time::now() - imu_time_).toSec() > timeout_)
    {
      imu_active_ = false;  imu_initializing_ = false;
      ROS_INFO("Imu sensor not active any more");
    }
    if ((vo_active_ || vo_initializing_) &&  (Time::now() - vo_time_).toSec() > timeout_)
    {
      vo_active_ = false;  vo_initializing_ = false;
      ROS_INFO("VO sensor not active any more");
    }

    if ((gps_active_ || gps_initializing_) &&  (Time::now() - gps_time_).toSec() > timeout_)
    {
