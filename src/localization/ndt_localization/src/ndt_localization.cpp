...
int ndt_num=0;
tf::Transform const_transform_odom_map;
...
bool NDTLocalization::init()
{
    ...
    if (param_use_odom_) 
    {
        ...
        sub_odom_imu_pose = nh_.subscribe<geometry_msgs::PoseStamped>("/imu_current_pose", 10, boost::bind(&NDTLocalization::odom_imu_poseCB, this, _1));
    }
    pub_current_pose_ = nh_.advertise<geometry_msgs::PoseStamped>("/ndt/current_pose", 10);
    pub_PoseWithCovarianceStamped = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("/ndt/PoseWithCovarianceStamped", 10);
    pub_ndt_odom = nh_.advertise<nav_msgs::Odometry>("/ndt_odometry",10);
...
void NDTLocalization::initialPoseCB(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg)
{
    ...
    predict_pose_odom_=pre_pose_ = current_pose_ = pre_pose_odom_ = current_pose_odom_  = initial_pose_;
    pose_init_ = true;
...
void NDTLocalization::init_pose_with_param()
{
    ...
    predict_pose_odom_ = pre_pose_ = pre_pose_odom_ = current_pose_odom_ = current_pose_ = initial_pose_;
    added_pose = initial_pose_;
    pose_init_ = true;

    offset_odom_.reset();  //    x = 0.; y = 0.;  z = 0.;  roll = 0.; pitch = 0.; yaw = 0.;
    offset_imu_.reset();
    if (param_debug_) {
        rawodom_init_ = false;
    }
...
void NDTLocalization::odomCB(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    offset_odom_.yaw += msg->twist.twist.angular.z * diff_time;  //里程计位置的变化量
...
void NDTLocalization::odom_imu_poseCB(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    if(!pose_init_)
    {
        return;
    ...
    else
    {
        ...
        predict_pose_odom_.z = pre_pose_.z; 
    }
} 
...
void NDTLocalization::pointCloudCB(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
   ...
   //平移
    ...
    predict_pose_error_ = std::sqrt((ndt_pose.x - predict_ndt_pose.x) * (ndt_pose.x - predict_ndt_pose.x)
                                  ...
                                  +0*(predict_ndt_pose.yaw - ndt_pose.yaw)*(predict_ndt_pose.yaw - ndt_pose.yaw);  //此处的参数需要调整！！！！
    ...
    geometry_msgs::PoseWithCovarianceStamped msg_PoseWithCovarianceStamped;
    msg_PoseWithCovarianceStamped.header.stamp = msg->header.stamp; // current pose is under "map_frame"
    msg_PoseWithCovarianceStamped.header.frame_id = param_map_frame_;  //  map
    pose2GeometryPose(msg_PoseWithCovarianceStamped.pose.pose, current_pose_);


    pub_PoseWithCovarianceStamped.publish(msg_PoseWithCovarianceStamped);

    //std::cout << "current pose(x,y,z,roll,pitch,yaw): " << "(" << current_pose_.x << ", " << current_pose_.y << ", " << current_pose_.z << ", " << current_pose_.roll << ", " << current_pose_.pitch << ", " << current_pose_.yaw << ")" << std::endl;
    ...
    {
        ndt_num++;
        tmp_q3.setRPY(current_pose_.roll, current_pose_.pitch, current_pose_.yaw);
        tf::Transform transform_odom_map(tmp_q3, tf::Vector3(current_pose_.x, current_pose_.y, 0.0));
        const_transform_odom_map = transform_odom_map;
        odom_initial_x = current_pose_.x;
        odom_initial_y = current_pose_.y;
        odom_initial_yaw = current_pose_.yaw;
    }
    else
    {  //发布 
    ...
    ndt_odometry.pose.pose.position.x = current_pose_.x; //位置 方向 pose  还有pose的协方差矩阵
    ndt_odometry.pose.pose.position.y = current_pose_.y;
    ndt_odometry.pose.pose.position.z = 0.2;
    ndt_odometry.pose.covariance[0] = 5e-10; //x
    ndt_odometry.pose.covariance[7] = 5e-10; //y
    ndt_odometry.pose.covariance[14] = 5e-3; //z
    ndt_odometry.pose.covariance[21] = 1e-3;  //roll
    ndt_odometry.pose.covariance[28] = 1e-3;//pitch
    ndt_odometry.pose.covariance[35] = 1e3;//yaw

    ndt_odometry.twist.twist.linear.x = 0; //linear x
    ndt_odometry.twist.twist.angular.x = 0; //angular x

    ndt_odometry.twist.covariance[0] = 1e-1; //linear x
    ndt_odometry.twist.covariance[7] = 1e-1;
    ndt_odometry.twist.covariance[14] = 1e-1;
    ndt_odometry.twist.covariance[21] = 1e-1;
    ndt_odometry.twist.covariance[28] = 1e-1;
    ndt_odometry.twist.covariance[35] = 1e-1;

    tf::Quaternion tmp_q4;
    tmp_q4.setRPY(current_pose_.roll, current_pose_.pitch, current_pose_.yaw );
    tf::quaternionTFToMsg(tmp_q4, ndt_odometry.pose.pose.orientation);
        //速度
    pub_ndt_odom.publish(ndt_odometry);
